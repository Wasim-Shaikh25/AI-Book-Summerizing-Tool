...
def assign_hierarchy(headings: Sequence[FinalHeading]) -> List[FinalHeading]:
    """
    Input:
      List[FinalHeading]
    ...
    _write_json(logs_dir / "hierarchy_request.json", request_list)
    ...
    _write_json(logs_dir / "hierarchy_response.json", response_payload)
