...
import re
# import grok_client # Placeholder for actual Grok client

from src.config import GROK_API_KEY, GROK_MODEL, ACTIVE_MODEL, CORE_IDEAS_MAX_TOKENS, MASTER_SUMMARY_MAX_TOKENS, REWRITE_MAX_TOKENS
from src.core.grok.prompts.prompts import PROMPT_CORE_IDEAS, PROMPT_MASTER_BRAIN_COMBINE_BLOCKS, PROMPT_MASTER_BRAIN_FINAL, PROMPT_REWRITE_CHUNK
...
class Summarizer:
    def __init__(self, active_model: str = ACTIVE_MODEL):
        ...
        self.grok_model_name = GROK_MODEL

        if self.active_model == "GROK":
            if not GROK_API_KEY or GROK_API_KEY == "YOUR_GROK_API_KEY":
                logger.warning("GROK_API_KEY not found or is placeholder. Grok summarizer will return mock responses.")
            # Placeholder for Grok API initialization
            # self.grok_client = grok_client.GrokClient(api_key=GROK_API_KEY)
            logger.info(f"Initializing Summarizer with Grok model: {self.grok_model_name} (using mock responses due to missing API key)...")
        else:
    ...
    def _generate_text(self, prompt: str, max_tokens: int, stream: bool = False) -> str:
        """Helper to generate text using the active LLM (Grok - mock implementation)."""
        full_response = []
        try:
            if self.active_model == "GROK":
                # Mock response for Grok
                mock_output = f"# Chapter Title: Mock Grok Chapter\n\n**Summary:** This is a mock summary generated by Grok for the prompt: {prompt[:100]}...\n\n## Mock Topic\n- This is a mock bullet point.\n- Another mock point."
                full_response.append(mock_output)
                if stream:
                    sys.stdout.write(mock_output)
                    sys.stdout.write("\n")
                    sys.stdout.flush()
    ...
    def _remove_prompt_from_output(self, generated_text: str, prompt_template: str, master_brain: str, chunk: str) -> str:
        ...
        """
        # This method might need adjustment based on actual Grok output characteristics
        # For now, it's a direct copy from Gemini's, assuming similar output structure.
        ...
        content_start_patterns = [
            r"^# Chapter Title:", # Updated for Grok mock
            r"^\*\*Summary:\*\*",
            r"^## Mock Topic", # Updated for Grok mock
