...
def main() -> int:
    ...
    print("  section source = 15e chapter hierarchy (fallback 15d)")
    if max_pages > 0:
        print(f"  max_pages      = {max_pages} (test mode)")
    if max_sections > 0:
        print(f"  max_sections   = {max_sections} (test mode)")
    print(f"  export_docx    = {export_docx}")
    print("=" * 60)
    ...
    if skip_structure:
        ...
        hierarchy_path = Path(log_dir) / "15e_chapter_hierarchy.json"
        if not ultimate_path.exists():
            print(f"[!] Missing {ultimate_path}")
            return 1
    else:
        print("\n[1/2] Structure pipeline (PDF -> headings + 15d/15e)...", flush=True)
        if max_pages > 0:
        ...
        if logger is not None:
            ...
            hierarchy_path = Path(logger.run_dir) / "15e_chapter_hierarchy.json"
    ...
    if hierarchy_path and hierarchy_path.exists():
        ...
        print(
            f"      15e method={hmeta.get('assignment_method')} "
    ...
    print(f"\n[2/2] Generating notes via {_provider_label()} ({len(work)} units)...")
    ...
    for idx, sec in enumerate(work, start=1):
        ...
        user_prompt = build_section_user_prompt(
            ...
            heading=heading,
            source_text=str(sec["text"])[:max_source_chars],
        )
        text = client.chat(system=system, user=user_prompt, max_tokens=1800)
        if idx == 1:
            print(f"      first-call model={client.last_model_label()}", flush=True)
        if not text:
            print(f"      [!] empty response for section {idx}", flush=True)
            continue
        rewritten[str(sec.get("section_id") or idx)] = text.strip()
    ...
    if hierarchy and hierarchy.get("chapters"):
        chapter_blocks, toc_entries = chapter_blocks_from_hierarchy(hierarchy, rewritten)
