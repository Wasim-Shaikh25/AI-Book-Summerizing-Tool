...
def _page_limit(max_pages: int | None) -> int:
    if max_pages is None:
        ...
        max_pages = int(config.PIPELINE_MAX_PAGES or 0)
    return max(0, int(max_pages or 0))


def _pymupdf_extract_pages_dict(pdf_path: str, *, max_pages: int | None = None) -> List[Dict[str, Any]]:
    """
...
def extract_visual_elements(pdf_path: str, *, max_pages: int | None = None) -> List[Dict[str, Any]]:
    """
...
) -> Tuple[List[NormalizedLine], str, List[Dict[str, Any]]]:
    """
